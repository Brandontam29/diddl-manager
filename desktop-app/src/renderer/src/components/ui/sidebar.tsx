...
import { PanelLeftIcon } from "lucide-solid"; // Changed to lucide-solid

import { createIsMobile } from "@rendererhooks/createIsMobile";
...
function SidebarProvider(props: SidebarProviderProps) {
  ...
  // Use mergeProps to handle defaultOpen logic safely with reactivity
  const merged = mergeProps({ defaultOpen: true }, local);

  // Assuming createIsMobile returns a signal accessor: () => boolean
  const isMobile = createIsMobile();
  const [openMobile, setOpenMobile] = createSignal(false);

  // Internal state
  const [_open, _setOpen] = createSignal(merged.defaultOpen);

  // Derived open state (controlled vs uncontrolled)
  const open = createMemo(() => merged.open ?? _open());

  const setOpen = (value: boolean | ((value: boolean) => boolean)) => {
    const nextOpen = typeof value === "function" ? value(open()) : value;

    if (merged.onOpenChange) {
          ...
          fallback={
            <Sheet open={openMobile()} onOpenChange={setOpenMobile} {...rest}>
              ...
              >
                ...
                <div class="flex h-full w-full flex-col">{merged.children}</div>
              </SheetContent>
            </Sheet>
          }
        >
          <div
            ...
            data-state={state()}
            data-collapsible={state() === "collapsed" ? merged.collapsible : ""}
...
function SidebarInput(props: ComponentProps<typeof TextField>) {
  const [local, rest] = splitProps(props, ["class"]);
  return (
    <TextField
...
function SidebarMenuButton(
  ...
  // Use Dynamic for polymorphism instead of Slot/asChild
  const ButtonComp = (
    <Dynamic
      component={local.as || "button"}
  ...
  return (
    <Show when={local.tooltip} fallback={ButtonComp}>
      <Tooltip>
        <TooltipTrigger>
          {/* TooltipTrigger in some Solid libs needs direct function or element, adjusting for general case */}
          {ButtonComp}
        </TooltipTrigger>
        <TooltipContent
          side="right"
...
function SidebarMenuAction(
  ...
  const [local, rest] = splitProps(props, ["as", "showOnHover", "class"]);
...
function SidebarMenuSubButton(
  ...
  const [local, rest] = splitProps(props, ["as", "size", "isActive", "class"]);
  const merged = mergeProps({ size: "md" as const }, local);
