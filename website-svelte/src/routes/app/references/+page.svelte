<script lang="ts">
	...
	async function runClientMutation() {
		...
		mutationResult = null;
		try {
			// Mutation example (commented out as conferences table is deleted)
			/*
			...
			*/
			mutationResult = 'Example mutation disabled (conferences table deleted)';
		} catch (err) {
	...
	<main class="mx-auto max-w-3xl space-y-6 px-6 py-8">
		...
		<section class="rounded-lg border border-stone-200 bg-white p-5">
			...
			<div class="mt-3 flex items-center gap-3">
				...
				>
					{mutationLoading ? 'Executing...' : 'Run Example Mutation'}
				</button>
				{#if mutationResult}
